    ...
    try {
        ...
        foreach ($policy in $selectedPolicies) {
            try {
                ...
                # Get current policy to preserve settings
                $currentPolicy = Get-MgIdentityConditionalAccessPolicy -ConditionalAccessPolicyId $policy.Id
                
                # Create update body with different structure based on operation
                $bodyParams = @{
                    conditions = @{
                        users = @{}
                    }
                ...
                # Get the full current policy to understand its structure
                Write-Verbose "Current policy conditions: $(ConvertTo-Json $currentPolicy.Conditions.Users -Depth 10)"

                if ($operation -eq 'Include') {
                    # For Include operation
                    $bodyParams.conditions.users = @{
                        includeGuestsOrExternalUsers = $guestConfig
                        includeUsers = @()  # Empty array for includeUsers
                    }

                    # Preserve existing exclude configurations except guest-related ones
                    if ($currentPolicy.Conditions.Users.ExcludeUsers) {
                        $bodyParams.conditions.users['excludeUsers'] = $currentPolicy.Conditions.Users.ExcludeUsers
                    }
                    if ($currentPolicy.Conditions.Users.ExcludeGroups) {
                        $bodyParams.conditions.users['excludeGroups'] = $currentPolicy.Conditions.Users.ExcludeGroups
                    }
                    if ($currentPolicy.Conditions.Users.ExcludeRoles) {
                        $bodyParams.conditions.users['excludeRoles'] = $currentPolicy.Conditions.Users.ExcludeRoles
                    }
                }
                else {
                    # For Exclude operation
                    $bodyParams.conditions.users = @{
                        excludeGuestsOrExternalUsers = $guestConfig
                    }

                    # Preserve existing include configurations except guest-related ones
                    if ($currentPolicy.Conditions.Users.IncludeUsers) {
                        $bodyParams.conditions.users['includeUsers'] = $currentPolicy.Conditions.Users.IncludeUsers
                    }
                    if ($currentPolicy.Conditions.Users.IncludeGroups) {
                        $bodyParams.conditions.users['includeGroups'] = $currentPolicy.Conditions.Users.IncludeGroups
                    }
                    if ($currentPolicy.Conditions.Users.IncludeRoles) {
                        $bodyParams.conditions.users['includeRoles'] = $currentPolicy.Conditions.Users.IncludeRoles
                    }
                }
