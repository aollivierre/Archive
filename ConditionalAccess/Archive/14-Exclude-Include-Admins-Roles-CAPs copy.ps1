...
function Get-ConditionalAccessPoliciesDetails {
    ...
    try {
        ...
        $formattedPolicies = foreach ($policy in $policies) {
            ...
            # Get role details for better display
            $includeRoles = if ($policy.Conditions.Users.IncludeRoles) {
                $roleCount = $policy.Conditions.Users.IncludeRoles.Count
                $roleText = if ($roleCount -eq 1) { "role" } else { "roles" }
                "Include: $roleCount $roleText"
            } else { "" }
            
            $excludeRoles = if ($policy.Conditions.Users.ExcludeRoles) {
                $roleCount = $policy.Conditions.Users.ExcludeRoles.Count
                $roleText = if ($roleCount -eq 1) { "role" } else { "roles" }
                "Exclude: $roleCount $roleText"
            } else { "" }
            
            $currentRoles = @($includeRoles, $excludeRoles) | Where-Object { $_ } | Join-String -Separator ", "
            ...
            [PSCustomObject]@{
                ...
                State = $policy.State
                Version = $version
                CurrentAdminRoles = if ($currentRoles) { $currentRoles } else { "No admin roles configured" }
            }
        }
        
        return $formattedPolicies | Sort-Object DisplayName
    }
    catch {
...
function Update-ConditionalAccessPolicyAdminRoles {
    ...
    try {
        ...
        Write-Host "`nSelected Policies ($($selectedPolicies.Count)):" -ForegroundColor White
        foreach ($policy in $selectedPolicies) {
            Write-Host "- $($policy.DisplayName)" -ForegroundColor Gray
            Write-Host "  Current: $($policy.CurrentAdminRoles)" -ForegroundColor DarkGray
        }
        ...
        foreach ($policy in $selectedPolicies) {
            try {
                ...
                # Build the complete body parameter
                $bodyParams = @{
                ...
                Write-Host "Successfully updated policy: $($policy.DisplayName) -> $newDisplayName" -ForegroundColor Green
            }
            catch {
